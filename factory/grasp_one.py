"""
grasp_one.py — Runs ONE grasp in a fresh Isaac Sim session.

Reads the grasp configuration from environment variables (set by run_factory.sh):
  GRASP_LABEL        e.g. "cylinder_center"
  GRASP_X            object grasp X position (world, meters)
  GRASP_Y            object grasp Y position (world, meters)
  GRASP_Z            object grasp Z position (world, meters)
  GRASP_APPROACH     approach height above grasp point (meters)
  GRASP_CLOSE_RAD    gripper close target (radians)
  GRASP_OUTPUT_DIR   folder to write the 6 CSV files into
  GRASP_BASENAME     filename prefix, e.g. "grasp_001_cylinder_center"

This script launches Isaac Sim ONCE, runs ONE grasp, writes the CSVs, and exits.
The bash loop in run_factory.sh calls it repeatedly with different poses.
Because each grasp gets a fresh Isaac Sim session, the TSF extension opens its
CSV files cleanly with the correct base name every time.

DO NOT run this directly with different poses in a loop yourself —
use run_factory.sh which sets the environment variables for you.
"""
...
# ============================================================
LABEL        = os.environ.get("GRASP_LABEL",      "cylinder_default")
...
OUTPUT_DIR   = os.environ.get("GRASP_OUTPUT_DIR", "/home/kourosh/Paper3_Simulation/Data/default_run")
BASENAME     = os.environ.get("GRASP_BASENAME",   "grasp_default")

print(f"[grasp_one] label={LABEL}")
print(f"[grasp_one] grasp world=({GRASP_X}, {GRASP_Y}, {GRASP_Z})  approach={APPROACH_H}")
print(f"[grasp_one] close={CLOSE_RAD}  output={OUTPUT_DIR}  basename={BASENAME}")
...
# ============================================================
# Enable TSF extension with THIS grasp's output dir + basename.
# Because this is a fresh session, the extension opens its files
# cleanly with the correct name when recording first starts.
# ============================================================
...
print(f"[grasp_one] TSF_85_Ext enabled={_ok}")
    ...
    for p in Usd.PrimRange(root_prim):
        if p.GetName() == "base_link":
            ...
            ROBOT_WORLD_QUAT_WXYZ[:] = [q.GetReal(),
                                         ...
                                         q.GetImaginary()[1],
                                         q.GetImaginary()[2]]
            print(f"[grasp_one] base_link world pos: {ROBOT_WORLD_POS}")
            break
...
# cuRobo
# ============================================================
print("[grasp_one] Loading cuRobo...")
ta = TensorDeviceType()
...
try:
    ...
    if traj_up is None:
        ...
        if traj_dn is None:
            ...
            print(f"[grasp_one] [RECORD ON] close -> hold -> open ...")
            ...
            print(f"[grasp_one] [RECORD OFF]")

            # ascent (not recorded)
            print(f"[grasp_one] straight ascent GRASP->UP ...")
            dz_up = float(np.linalg.norm(up_world - grasp_world))
            traj_up2 = plan_stitched_z(q_grasp, dz_up, f"{LABEL}:UP")
            if traj_up2 is not None:
                run_traj(traj_up2)

            print(f"[grasp_one] SUCCESS. Data in {OUTPUT_DIR} (prefix {BASENAME})")
finally:
    # Let any final CSV flush happen
    for _ in range(30):
        world.step(render=True)
