...
_CAL = _load_cal()
# CALIBRATION IS KEYED BY WIDTH **AND SHAPE** (2026-08-28).
# A 20 mm cuboid and a 20 mm cylinder close the jaw to the same gap, so their
# FIXED close_rad is the same -- but contact-aware is not: a flat face gives
# the pads far more area, trips the target earlier, and stores a different
# stopping angle. Under a width-only key those two overwrite each other
# silently, and the throat entry (measured AT that close_rad) inherits the
# error. Cylinders keep the bare width key so every existing file and every
# run made so far still resolves; other shapes get their own.
def cal_key(width_mm, shape="cylinder"):
    sh = (shape or "cylinder").lower()
    return (f"{float(width_mm):.1f}" if sh == "cylinder"
            else f"{float(width_mm):.1f}|{sh}")


_OBJ_SHAPE_EARLY = os.environ.get("GRASP_OBJECT_SHAPE", "cylinder").strip().lower()
_diam_key = cal_key(OBJ_DIAM_MM, _OBJ_SHAPE_EARLY)
_diam_key_fallback = f"{OBJ_DIAM_MM:.1f}"
...
if os.environ.get("GRASP_TOOL_OFFSET"):            # manual override (dev)
    ...
    # NO CROSS-SHAPE FALLBACK (2026-08-28). Borrowing the cylinder's entry for
    # a cuboid of the same width was allowed here with a warning, but a warning
    # in a 400-line log is not a decision: a fixed-angle close_rad does
    # transfer between shapes (same jaw gap), while a CONTACT-AWARE one does
    # not, because a flat face trips the target far earlier than a round one.
    # Since the file does not record which method produced an entry, the safe
    # answer is to refuse and say what is missing.
    print(f"\n[cal] REFUSING TO RUN: no {_OBJ_SHAPE_EARLY} calibration for "
          f"{OBJ_DIAM_MM:.1f} mm (looked for key '{_diam_key}').")
    if _diam_key_fallback in _CAL and _OBJ_SHAPE_EARLY != "cylinder":
        print(f"[cal] A CYLINDER entry for {OBJ_DIAM_MM:.1f} mm does exist, and "
              f"it is NOT used: the jaw gap matches but a contact-aware "
              f"close_rad measured on a round face is the wrong number for a "
              f"flat one. Calibrate this shape.")
...
elif _diam_key_fallback in _CAL and "close_rad" in _CAL[_diam_key_fallback]:
    CLOSE_RAD = float(_CAL[_diam_key_fallback]["close_rad"])
    print(f"[cal] !! close_rad taken from the CYLINDER entry for "
          f"{OBJ_DIAM_MM:.1f} mm (no {_OBJ_SHAPE_EARLY} entry)")
...
# GROUND PLANE WITHOUT THE INTERNET (2026-08-27).
# add_default_ground_plane() fetches an asset from Nvidia's S3 bucket. With no
# route to it, omni.client blocks for ~5 MINUTES and then raises
# "Could not find assets root folder", which kills the run before the scene
# exists -- an empty Isaac window and a stack trace that names the ground
# plane, not the network. It happened on 2026-08-27 to a plain scaled-cylinder
# run, i.e. nothing to do with what was being grasped.
#
# The floor only has to be flat and solid: cuRobo plans against the TABLE SLAB
# in its own world model, not against this. So a locally authored static
# collider does the job with no download and no failure mode.
try:
    world.scene.add_default_ground_plane()
    print("[cfg] ground plane: Nvidia default asset")
except Exception as _gp_err:
    from pxr import UsdGeom as _UG, UsdPhysics as _UP, Gf as _GF
    _gp = "/World/localGroundPlane"
    _prim = _UG.Cube.Define(world.stage, _gp)
    _prim.GetSizeAttr().Set(1.0)
    _x = _UG.Xformable(_prim.GetPrim())
    _x.ClearXformOpOrder()
    _x.AddTranslateOp().Set(_GF.Vec3d(0.0, 0.0, -0.5))     # top face at z = 0
    _x.AddScaleOp().Set(_GF.Vec3d(50.0, 50.0, 1.0))        # 50 m square, 1 m thick
    _UP.CollisionAPI.Apply(_prim.GetPrim())
    print(f"[cfg] ground plane: Nvidia asset unreachable "
          f"({type(_gp_err).__name__}), using a local 50 m static collider "
          f"instead — this is a NETWORK problem, not a scene problem")
...
# ============================================================
# ---- EXTERNAL OBJECT USD (2026-08-27) -----------------------------------
# The scaled unit cylinder above covers cylinders and nothing else. Berith's
# object set is 23 authored meshes -- cuboids, cubes, spheres -- so those are
# REFERENCED IN under Object_02 instead, which keeps the placement, the fixed
# joint and the obstacle wiring exactly as they are and changes only what is
# hanging there.
#
# TWO THINGS THAT ARE NOT OPTIONAL:
#
# 1. RE-CENTRING. 19 of the 23 are modelled sitting on their base, geometry
#    centred at +50 mm in Z (or +9, +4, +30, +45). Everything here places the
#    object by CENTRE, so the offset is subtracted on load. Skip it and every
#    grasp lands 50 mm off while the preview, the logs and the ledger all look
#    perfectly normal -- the worst kind of wrong.
#
# 2. COLLISION. A referenced art mesh may carry no physics schema at all, in
#    which case the gripper closes straight through it and the tactile map is
#    empty for a reason that looks nothing like a jam. CollisionAPI is applied
#    if missing, and what was found is printed either way.
_OBJ_MESH_PATH = "/World/robot_gripper_adapter_sensor/Object_02/Cylinder"
_OBJ_EXT_PATH  = "/World/robot_gripper_adapter_sensor/Object_02/ExtObject"
_OBJ_USD       = os.environ.get("GRASP_OBJECT_USD", "").strip()
# SCALED CUBOID: no mesh file at all, just a box primitive sized from the GUI.
# Same idea as the scaled cylinder -- the scene's unit mesh becomes whatever
# size is typed -- so any Paper 1 geometry can be made without waiting for
# someone to author a USD for it. Dimensions are world X (the jaw closes on
# this), world Y (across the pad), world Z (along the pad).
_OBJ_SHAPE     = os.environ.get("GRASP_OBJECT_SHAPE", "cylinder").strip().lower()
_OBJ_BOX_MM    = [float(v) for v in
                  os.environ.get("GRASP_OBJECT_BOX_MM", "40,18,100").split(",")]
_OBJ_CENTRE_MM = [float(v) for v in
                  os.environ.get("GRASP_OBJECT_CENTRE_MM", "0,0,0").split(",")]
_obj_len_mm = float(CONFIG["object"].get("length_mm", 140.0))

if (not _OBJ_USD) and _OBJ_SHAPE in ("cuboid", "cube"):
    try:
        from pxr import (Gf as _Gf, UsdGeom as _UsdGeom,
                         UsdPhysics as _UsdPhys, Sdf)
        _bx, _by, _bz = [v / 1000.0 for v in _OBJ_BOX_MM]
        _cube = _UsdGeom.Cube.Define(stage, _OBJ_EXT_PATH)
        _cube.GetSizeAttr().Set(1.0)                 # unit cube, then scaled
        _cx = _UsdGeom.Xformable(_cube.GetPrim())
        _cx.ClearXformOpOrder()
        # base on Object_02's origin, centred in x and y -- the same
        # convention the authored unit cylinder uses
        _cx.AddTranslateOp().Set(_Gf.Vec3d(0.0, 0.0, _bz / 2.0))
        _cx.AddScaleOp().Set(_Gf.Vec3d(_bx, _by, _bz))
        _UsdPhys.CollisionAPI.Apply(_cube.GetPrim())
        _old = stage.GetPrimAtPath(_OBJ_MESH_PATH)
        if _old and _old.IsValid():
            _old.SetActive(False)
        print(f"[obj] SCALED {_OBJ_SHAPE.upper()}: "
              f"{_OBJ_BOX_MM[0]:g} (grip, X) x {_OBJ_BOX_MM[1]:g} (across, Y) "
              f"x {_OBJ_BOX_MM[2]:g} (along, Z) mm — a box primitive, no mesh "
              f"file involved")
    except Exception as _e:
        print(f"[obj] !! SCALED CUBOID FAILED ({type(_e).__name__}: {_e}). "
              f"Falling back to the authored unit cylinder.")
        _OBJ_SHAPE = "cylinder"
...
if _OBJ_USD:
    try:
        from pxr import (Gf as _Gf, UsdGeom as _UsdGeom,
                         UsdPhysics as _UsdPhys, Sdf)
        if not os.path.isfile(_OBJ_USD):
            raise FileNotFoundError(_OBJ_USD)
        from isaacsim.core.utils.stage import add_reference_to_stage as _addref

        # An Xform WE own, holding the re-centre. The USD is referenced into a
        # CHILD of it, so the referenced prim's own transform is never touched.
        # (2026-08-27: clearing the referenced prim's xformOpOrder to add the
        # translate also threw away its authored orientation, and Cylinder_M
        # came up lying on its side.)
        _root = _UsdGeom.Xform.Define(stage, _OBJ_EXT_PATH)
        _root.ClearXformOpOrder()
        _t_op = _root.AddTranslateOp()
        _t_op.Set(_Gf.Vec3d(0.0, 0.0, 0.0))
        _child = _OBJ_EXT_PATH + "/Geom"
        _addref(usd_path=_OBJ_USD, prim_path=_child)
        _ext = stage.GetPrimAtPath(_child)
        if not _ext or not _ext.IsValid():
            raise RuntimeError(f"reference did not create {_child}")

        # PLACEMENT IS MEASURED, NOT TAKEN FROM THE LIBRARY (2026-08-27).
        # The scene's convention is that Object_02's ORIGIN IS THE OBJECT'S
        # BASE -- the authored unit cylinder is shifted up by L/2 internally
        # for exactly that reason. Berith's meshes are already modelled sitting
        # on their base, so they were in the right convention to begin with,
        # and subtracting their centre offset pushed them 50 mm DOWN through
        # the floor. Measured: centre landed at z = 982.2 mm against an
        # intended 1052.2. It also desynced Object_02's WorldFixedJoint, which
        # PhysX reported as "disjointed body transforms", and the object then
        # settled and toppled -- the horizontal, wobbling cylinder.
        #
        # So instead of trusting a number: read the geometry's own bounds in
        # this wrapper's frame and shift it so its base sits at z = 0 and it is
        # centred in x and y. That reproduces the unit-mesh convention exactly
        # and cannot be wrong about which end is the base.
        _bbc0 = _UsdGeom.BBoxCache(Usd.TimeCode.Default(),
                                   [_UsdGeom.Tokens.default_,
                                    _UsdGeom.Tokens.render])
        _lr = _bbc0.ComputeUntransformedBound(_ext).ComputeAlignedRange()
        if _lr.IsEmpty():
            raise RuntimeError("referenced geometry has no bounds")
        _lmn, _lmx = _lr.GetMin(), _lr.GetMax()
        _shift = _Gf.Vec3d(-0.5 * (_lmn[0] + _lmx[0]),
                           -0.5 * (_lmn[1] + _lmx[1]),
                           -_lmn[2])
        _t_op.Set(_shift)

        # NESTED RIGID BODIES ARE THE BUG (2026-08-27). Object_02 already IS
        # the rigid body -- it is the prim the scene applies RigidBodyAPI to
        # and bolts in place. A referenced object that brings its OWN
        # RigidBodyAPI becomes a body inside a body, which PhysX does not
        # define, and the result was an object that fell to the floor, tipped
        # over and slid around before the fingers ever reached it.
        # Stripping the API leaves the geometry as collision shapes belonging
        # to Object_02, which is what the pipeline has always assumed.
        # WHAT IS ACTUALLY IN THE FILE. Printed, not assumed: two attempts to
        # fix this by reasoning about what Berith's USDs "probably" contain
        # both failed, and the object kept toppling. This lists every prim and
        # the schemas applied to it, so the next decision is made from the
        # stage rather than from a guess.
        print("[obj]   --- referenced prims and their applied schemas ---")
        _n = 0
        for q in Usd.PrimRange(_ext):
            try:
                _ap = list(q.GetAppliedSchemas())
            except Exception:
                _ap = ["<unreadable>"]
            if _n < 25:
                print(f"[obj]     {q.GetPath().pathString[-58:]:<58} "
                      f"{q.GetTypeName():<12} {_ap}")
            _n += 1
        if _n > 25:
            print(f"[obj]     ... and {_n - 25} more prim(s)")

        # DISABLE, do not merely un-apply. RemoveAPI is not reliable across USD
        # builds and can report success without changing what PhysX parses, so
        # the authoritative attribute is written directly and CREATED if it is
        # absent. Belt and braces: the API is also removed where that works.
        _stripped = 0
        for q in Usd.PrimRange(_ext):
            _is_rb = False
            try:
                _is_rb = q.HasAPI(_UsdPhys.RigidBodyAPI)
            except Exception:
                pass
            if not _is_rb:
                _is_rb = any("RigidBodyAPI" in a
                             for a in (q.GetAppliedSchemas() or []))
            if _is_rb:
                try:
                    _at = q.GetAttribute("physics:rigidBodyEnabled")
                    if not _at or not _at.IsValid():
                        _at = _UsdPhys.RigidBodyAPI(q).CreateRigidBodyEnabledAttr()
                    _at.Set(False)
                    _stripped += 1
                except Exception as _re:
                    print(f"[obj]   !! could not disable rigid body on "
                          f"{q.GetPath()}: {_re}")
                try:
                    q.RemoveAPI(_UsdPhys.RigidBodyAPI)
                except Exception:
                    pass
            try:
                if q.HasAPI(_UsdPhys.ArticulationRootAPI):
                    q.RemoveAPI(_UsdPhys.ArticulationRootAPI)
            except Exception:
                pass
        # a joint inside the referenced object would also fight Object_02
        for q in Usd.PrimRange(_ext):
            if q.IsA(_UsdPhys.Joint):
                q.SetActive(False)
                print(f"[obj]   joint deactivated inside the object: "
                      f"{q.GetPath()}")

        _old = stage.GetPrimAtPath(_OBJ_MESH_PATH)
        if _old and _old.IsValid():
            _old.SetActive(False)
            print("[obj] authored unit cylinder deactivated")

        _meshes = [q for q in Usd.PrimRange(_ext) if q.IsA(_UsdGeom.Mesh)]
        _had = [q for q in _meshes if q.HasAPI(_UsdPhys.CollisionAPI)]
        for q in _meshes:
            if not q.HasAPI(_UsdPhys.CollisionAPI):
                _UsdPhys.CollisionAPI.Apply(q)
            try:
                _mc = (_UsdPhys.MeshCollisionAPI(q)
                       if q.HasAPI(_UsdPhys.MeshCollisionAPI)
                       else _UsdPhys.MeshCollisionAPI.Apply(q))
                _mc.CreateApproximationAttr().Set("convexHull")
            except Exception:
                pass
        print(f"[obj] EXTERNAL USD: {os.path.basename(_OBJ_USD)}")
        print(f"[obj]   placed by MEASUREMENT: shifted "
              f"({_shift[0]*1000:+.1f}, {_shift[1]*1000:+.1f}, "
              f"{_shift[2]*1000:+.1f}) mm so its BASE sits on Object_02's "
              f"origin — the same convention as the authored unit cylinder. "
              f"GRASP_OBJECT_CENTRE_MM is ignored.")
        print(f"[obj]   {_stripped} nested rigid body(ies) DISABLED — the "
              f"geometry now belongs to Object_02's body")
        if _stripped == 0:
            print("[obj]   (none found; if the object still topples, the "
                  "cause is NOT a nested rigid body — read the schema dump "
                  "above)")
        print(f"[obj]   {len(_meshes)} mesh(es); {len(_had)} already had "
              f"CollisionAPI, {len(_meshes) - len(_had)} given it now")
        if not _meshes:
            print("[obj]   !! NO MESH PRIMS FOUND — the gripper will close on "
                  "nothing and every map will be empty.")
        try:
            _bb = _UsdGeom.BBoxCache(Usd.TimeCode.Default(),
                                     [_UsdGeom.Tokens.default_])
            _rg = _bb.ComputeWorldBound(_ext).ComputeAlignedRange()
            _mn, _mx = _rg.GetMin(), _rg.GetMax()
            _d = [(_mx[i] - _mn[i]) * 1000.0 for i in range(3)]
            _c = [((_mx[i] + _mn[i]) / 2.0) * 1000.0 for i in range(3)]
            print(f"[obj]   as placed: {_d[0]:.1f} x {_d[1]:.1f} x {_d[2]:.1f} mm, "
                  f"centre ({_c[0]:.1f}, {_c[1]:.1f}, {_c[2]:.1f}) mm")
            if _d[2] < max(_d[0], _d[1]) - 1.0:
                print(f"[obj]   !! the long axis is NOT vertical — this object "
                      f"is lying down. Expected {max(_d):.0f} mm in Z.")
        except Exception:
            pass
    except Exception as _e:
        print(f"[obj] !! EXTERNAL USD FAILED ({type(_e).__name__}: {_e}). "
              f"Falling back to the authored unit cylinder.")
        _OBJ_USD = ""
...
try:
    ...
    if _OBJ_SHAPE in ("cuboid", "cube") and not _OBJ_USD:
        print(f"[obj] size comes from the scaled box, not from the unit "
              f"cylinder")
    elif _OBJ_USD:
        print(f"[obj] size comes from the referenced USD, not from scaling "
              f"(D={OBJ_DIAM_MM:.1f} mm, L={_obj_len_mm:.1f} mm are used only "
              f"for the grid limits)")
    elif not _mesh.IsValid():
...
if os.environ.get("GRASP_FREEZE_OBJECT", "1") != "0":
    ...
    try:
        ...
        if _obj.IsValid():
            ...
            # PLACE BY MEASUREMENT, NOT BY INHERITANCE (2026-08-27).
            # _pz above is Object_02's AUTHORED position, and the scene authored
            # it as the base of a 140 mm rod. The object's CENTRE therefore
            # drifts with length: a 100 mm body lands 20 mm low
            # (measured: centre 1032.2 against an intended 1052.2), and the same
            # would happen to a scaled cylinder at any length but 140.
            #
            # So: put the object where it is asked to be. Measure the geometry's
            # actual world bounds, then shift the prim by whatever it takes to
            # bring that centre onto OBJ_CENTER. At L = 140 the correction is
            # 0.0 mm, so every run made so far is unchanged.
            if "OBJ_POS_Z" not in os.environ:
                try:
                    _bbc1 = UsdGeom.BBoxCache(Usd.TimeCode.Default(),
                                              [UsdGeom.Tokens.default_,
                                               UsdGeom.Tokens.render])
                    _wr = _bbc1.ComputeWorldBound(_obj).ComputeAlignedRange()
                    if not _wr.IsEmpty():
                        _wc = 0.5 * (_wr.GetMin()[2] + _wr.GetMax()[2])
                        _dz_fix = float(OBJ_CENTER[2]) - float(_wc)
                        if abs(_dz_fix) > 1e-6:
                            print(f"[grid] centring: geometry centre measured "
                                  f"at {_wc*1000:.1f} mm, target "
                                  f"{OBJ_CENTER[2]*1000:.1f} mm -> shifting the "
                                  f"object by {_dz_fix*1000:+.1f} mm")
                        _pz = _pz + _dz_fix
                except Exception as _ce2:
                    print(f"[grid] note: could not measure the object for "
                          f"centring ({_ce2}); using the authored pose")
