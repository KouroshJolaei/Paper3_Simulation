...
# FRAMES. How many of a grasp's ~250 recorded frames actually landed inside
# the hold window. hold_average returns this and every caller threw it away,
# which is how run_20260903_173806 pt43 reached the stitch and the training
# pairs with a "hold average" built from ONE frame -- the release transient.
# Two independent tests, because they fail in different ways:
#   RATIO   s1 vs s2 at the same point. Catches one sensor collapsing while
#           the other is healthy, which is the common case.
#   FLOOR   an absolute minimum. The ratio test is BLIND when both sensors
#           collapse together (pt41 came through as 1/14 and only fired
#           because 14 > 2*1). A healthy point holds ~210 frames, so 50 is
#           far below anything normal and far above any collapse seen.
FRAME_RATIO_MAX = 2.0
FRAME_MIN = 50
...
def collect(run_dir, sensor="s1", want_blob=False, band_width_mm=8.0):
    ...
    # the OTHER sensor, read only for its frame counts: a collapsed hold
    # window shows up as a disagreement between the two pads long before it
    # shows up in any single-sensor number
    ...
    for p in led.get("points", []):
        ...
        _o_ps, _o_pm, nfo = peaks_o.get(tag, (None, None, None))
...
def verdicts(rows):
    ...
    for r in rows:
        ...
        if nf is not None and nf < FRAME_MIN:
            tags.append(f"FRAMES-LOW {nf}")
        elif (nf and nfo
              and max(nf, nfo) > FRAME_RATIO_MAX * min(nf, nfo)):
...
def grid_quality_report(run_dir, sensor="s1", want_blob=False,
    ...
    if not bad:
        ...
        for r in sorted(bad, key=lambda r: r["index"]):
            ...
            fr = ("  --" if r.get("hold_frames") is None
                  else f"{r['hold_frames']:4d}")
            L.append(f"     {r['tag']}  {r['y']:+6.1f} {r['z']:+6.2f}"
                     f"   {dm}    {sf}   {ps}  {fr}   {r['verdict']}")
        L.append("")
    ...
    L.append("")
    L.append("frms      = frames inside the hold window, of the ~250 recorded.")
    L.append("            A healthy grasp holds ~210. A LOW COUNT DOES NOT MEAN")
    L.append("            FRAMES WERE LOST -- they are all still in the CSV. It")
    L.append("            means the window selected almost none of them, so that")
    L.append("            grasp's map is a snapshot rather than an average.")
    L.append("")
