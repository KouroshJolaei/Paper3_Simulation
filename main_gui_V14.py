...
def design_grid(diameter_mm, length_mm, tool_offset_z_mm, step_mm=6.0,
    ...
    def _worst(n_across_, n_along_, dz_):
        ...
        gap = grid_gripper_gap_mm(offs, dz_, tool_offset_z_mm,
                                  pad_roll_deg, cloud)
    ...
    def _lowest_clear_dz(n_across_, n_along_):
        """Smallest base height at which no point touches the body.
        Raising the tool always moves the palm away from a rod that stands
        below it, so the gap rises monotonically with dz and a bisection is
        exact to the tolerance. Returns None if even the top of the search
        range collides."""
        lo, hi = dz_lo - 60.0, dz_hi + 60.0
        if _worst(n_across_, n_along_, hi)[0] < palm_clear_mm:
            return None
        for _ in range(40):
            mid = 0.5 * (lo + hi)
            if _worst(n_across_, n_along_, mid)[0] >= palm_clear_mm:
                hi = mid
            else:
                lo = mid
            if hi - lo < 0.01:
    ...
    while True:
        ...
        if need is not None:
            ...
            if need <= dz_end + 1e-9:
                ...
                if g >= palm_clear_mm and o <= 1e-6:
                    if abs(chosen - dz_mid) > 0.01:
                        why.append(
                            f"        base height moved {dz_mid:+.2f} -> "
                            f"{chosen:+.2f} mm: the midpoint left "
                            + ("the palm short" if chosen > dz_mid
    ...
    why.append(f"CHECK : every point verified against the gripper body "
               f"(palm {PALM_DROP_MM:.2f} mm past the flange, radius "
               f"{PALM_RADIUS_MM:.0f} mm)")
    why.append(f"        worst rod-to-gripper gap {gap.min():+.2f} mm "
               f"(need {palm_clear_mm:.1f}); worst pad overhang past the rod "
