...
def design_grid(diameter_mm, length_mm, tool_offset_z_mm, step_mm=6.0,
                ...
                obj_tilt_axis="X", shape="cylinder", band_width_mm=None,
                end_overhang=False):
    ...
    # ---- ACROSS -----------------------------------------------------------
    # A CUBOID'S BAND COMES FROM ITS ACROSS-PAD EXTENT, NOT THE GRASPED WIDTH.
    # Berith's cuboids are all 40 mm in the closing direction and vary 8/18/40
    # ACROSS the pad, so using the grasped width would give all five the same
    # band and miss the only thing that differs between them. A cylinder is
    # round, so the two are the same number and this changes nothing.
    band, band_along = contact_band_mm(shape,
    ...
    # ASYMMETRY, REMOVED (2026-08-27). Across, the pad may hang off the edge as
    # long as across_margin_mm still touches -- that is what puts an EDGE in
    # the map. Along, it had to stay wholly on the body, so the END of an
    # object could never be seen at ANY length. Geometry never forbade it: the
    # throat floor sits below this ceiling for every length. end_overhang
    # applies the same rule to the ends; default off, so old designs stand.
    dz_hi = ((L / 2.0) + pad_hh - across_margin_mm if end_overhang
             else (L / 2.0) - pad_hh)                    # highest allowed
    ...
    if end_overhang:
        why.append(f"        END OVERHANG ON: the pad may hang past the end "
                   ...
                   f"same rule the across direction already uses, and the only "
                   f"way an END shows up in the map")
    ...
    def _worst(n_across_, n_along_, dz_):
        ...
        gap = _gaps(offs, dz_)
        # with end_overhang the pad is ALLOWED past the end, up to the point
        # where only across_margin_mm of it still touches
        _allow = (2 * pad_hh - across_margin_mm) if end_overhang else 0.0
        over = grid_pad_overhang_mm(offs, dz_, pad_hh, L / 2.0) - _allow
    ...
    while True:
        ...
        if need is not None:
            ...
            dz_end = ((L / 2.0) + pad_hh - across_margin_mm if end_overhang
                      else (L / 2.0) - pad_hh) - gx_hi - 0.01
            if need <= dz_end + 1e-9:
    ...
    # WHERE TO PUT THE ACROSS ANCHOR so the sweep straddles the side edge.
    # Centring it (y = 0) leaves a wide face showing nothing but plateau; the
    # user picking a number by hand overshoots -- y = 45 on a 120 mm face put
    # the outer column's pad edge at 92 mm against an edge at 60, so those
    # columns touched nothing. The far column should land exactly ON the
    # across limit, where across_margin_mm of pad still touches:
    #     y = across_max - n_across * step
    # Clamped at 0, because a narrow object has no edge to reach out to.
    pad_dy_edge = max(0.0, across_max - n_across * step)
...
class CockpitGUI:
    def __init__(self, root):
        ...
        self.vars = {
            ...
            # scaled cuboid: world X (jaw closes on this), Y (across the pad),
            # Z (along the pad). Defaults are a Paper-1-sized block.
            "box_w": tk.StringVar(value="40.0"),
            "box_d": tk.StringVar(value="30.0"),
            "box_h": tk.StringVar(value="100.0"),
            # Let the pad hang past the ENDS, the same way it already may
            # across. Off by default so existing designs are unchanged.
            "end_overhang": tk.BooleanVar(value=False),
            # Put the across anchor where the sweep straddles the side edge.
            "auto_edge_y": tk.BooleanVar(value=False),
    ...
    def _build_inputs(self):
        ...
        # obj_diam IS the grasped width -- what the jaw closes on -- and W(grip)
        # feeds it, which looked like a bug when the box marked "diameter"
        # changed. The label now says which it is.
        for key, lab in [("obj_diam", "diameter / grip W (mm)"),
                         ("obj_len", "length / height (mm)")]:
            ttk.Label(frm, text=lab).grid(row=r, column=0, sticky="e")
            e = ttk.Entry(frm, textvariable=self.vars[key], width=12)
        ...
        self.box_frame = ttk.Frame(frm)
        self.box_frame.grid(row=r, column=0, columnspan=2, sticky="w"); r += 1
        ttk.Label(self.box_frame, text="cuboid  W(grip)").grid(row=0, column=0,
                                                               sticky="e")
        ttk.Entry(self.box_frame, textvariable=self.vars["box_w"],
                  width=6).grid(row=0, column=1)
        ttk.Label(self.box_frame, text="D(across)").grid(row=0, column=2,
                                                         sticky="e")
        ttk.Entry(self.box_frame, textvariable=self.vars["box_d"],
                  width=6).grid(row=0, column=3)
        ttk.Label(self.box_frame, text="H(along)").grid(row=0, column=4,
                                                        sticky="e")
        ttk.Entry(self.box_frame, textvariable=self.vars["box_h"],
                  width=6).grid(row=0, column=5)
        # DEBOUNCED, not per-keystroke (2026-08-27). Applying on every write
        # meant typing "140" redrew the object as 1 mm, then 14, then 140 —
        # the plots flickered through nonsense sizes on the way. 400 ms after
        # you stop typing is late enough to see the whole number and early
        # enough that no button press is needed.
        self._box_after = None

        def _box_changed(*_a):
            if self.vars["object_pick"].get() != self.OBJ_BOX:
                return
            if self._box_after:
                try:
                    self.root.after_cancel(self._box_after)
                except Exception:
                    pass
            self._box_after = self.root.after(400, self._obj_pick)

        for _v in ("box_w", "box_d", "box_h"):
            self.vars[_v].trace_add("write", _box_changed)
    ...
    OBJ_BOX  = "(scaled cuboid — type W x D x H)"
    ...
    def _picked_band_width(self):
        """The extent ACROSS the pad, which is what sets the contact band.

        Same as the grasped width for anything round; for a cuboid it is the
        other cross-section dimension."""
        raw = self.vars["object_pick"].get()
        if raw == self.OBJ_BOX:
            try:
                return float(self.vars["box_d"].get())
            except ValueError:
                return None
        name = getattr(self, "_obj_by_label", {}).get(raw)
        e = self._obj_library().get(name) if name else None
        return (e or {}).get("band_width_mm")
    ...
    def _obj_label(self, name, e):
        ...
        d = e.get("cross_depth_mm", w)
        size = (f"{w:g} x {L:g}" if abs(d - w) < 0.05
                else f"{w:g}(grip) x {d:g}(across) x {L:g}")
        return f"{stem:<16} {size} mm  {pose}{tag}"
    ...
    def _obj_choices(self):
        ...
        self._obj_by_label = {}
        out = [self.OBJ_NONE, self.OBJ_BOX]
        last_fam = None
    ...
    def _obj_pick(self, _e=None):
        ...
        if raw == self.OBJ_BOX:
            try:
                w = float(self.vars["box_w"].get())
                d = float(self.vars["box_d"].get())
                h = float(self.vars["box_h"].get())
            except ValueError:
                self.obj_pick_lbl.config(text="cuboid sizes must be numbers.",
                                         foreground="#a00")
                return
            # The cylinder's fields are NOT touched: box_w/box_h are this
            # shape's own size and _sync_object_size reads them directly.
            self._stand_on_table()
            self._obj_last_ok = raw
            self.obj_pick_lbl.config(
                text=(f"SCALED CUBOID {w:g} x {d:g} x {h:g} mm — a box "
                      f"primitive, no mesh file. Jaw closes on {w:g} mm "
                      f"(needs that width calibrated); the pad sees {d:g} mm "
                      f"across, which sets the contact band."),
                foreground="#070")
            try:
                self.refresh()
            except Exception:
    ...
    def _obj_env(self):
        ...
        if raw == self.OBJ_BOX:
            try:
                ...
                return ""
            return (f'GRASP_OBJECT_SHAPE="cuboid" \\\n'
                    f'GRASP_OBJECT_BOX_MM="{w:g},{d:g},{h:g}" \\\n')
        name = getattr(self, "_obj_by_label", {}).get(raw)
        e = self._obj_library().get(name) if name else None
    ...
    @staticmethod
    def _cal_key(shape, width_mm):
        """How a calibration entry is named: bare width for a cylinder (so
        every entry measured before shapes existed still reads), width|shape
        for anything else."""
        sh = (shape or "cylinder").lower()
    ...
    def _cal_entry(self):
        ...
        try:
            with open(self._cal_path()) as f:
                cal = json.load(f)
            sh = self._picked_shape()
            # NO CROSS-SHAPE FALLBACK (2026-08-28). Borrowing a cylinder's
            # entry for a cuboid of the same width is only safe for a
            # fixed-angle close_rad; a contact-aware one is measured against a
            # flat face and trips far earlier, so the two are different numbers
            # under the same width. Refusing is the honest answer.
            return cal.get(self._cal_key(sh, CYL_D))
        except Exception:
            return None
    ...
    def refresh(self):
        ...
        # (outline shape chosen just below)
        ax.set_xlabel("world X (mm)"); ax.set_ylabel("world Y (mm)")
        # THE OUTLINE FOLLOWS THE SHAPE (2026-08-27). A cuboid drawn as a
        # circle hides the one thing the top-down view is for: which face the
        # pads meet, and how much of it the pad spans. X is the grasped width,
        # Y is the across-pad extent.
        _shape = self._picked_shape()
        _across = self._picked_band_width() or CYL_D
        # W(grip) feeds obj_diam, which only updated when the field was
        # committed with Enter, so typing a new width left the outline stale.
        # A local, not CYL_D itself: assigning the module global inside this
        # method makes it local for the WHOLE method and every earlier read of
        # it raises UnboundLocalError.
        _gripW = CYL_D
        if self.vars["object_pick"].get() == self.OBJ_BOX:
            try:
                _gripW = float(self.vars["box_w"].get())
            except ValueError:
                pass
        if _shape in ("cuboid", "cube"):
            _bx, _by = _gripW / 2.0, float(_across) / 2.0
            ax.plot(obj[0] + np.array([-_bx, _bx, _bx, -_bx, -_bx]),
                    obj[1] + np.array([-_by, -_by, _by, _by, -_by]),
                    color="steelblue", linewidth=2,
                    label=f"{_shape} {_gripW:g} x {float(_across):g} mm")
        else:
            th = np.linspace(0, 2*np.pi, 80)
            ax.plot(obj[0] + (CYL_D/2)*np.cos(th), obj[1] + (CYL_D/2)*np.sin(th),
                    color="steelblue", linewidth=2, label=_shape)
        ...
        ax.set_title(f"FRONT (Y-Z)\npad face on {self._picked_shape()}, grid")
        ...
        # In the FRONT (Y-Z) view the horizontal axis is world Y, so the width
        # to draw is the ACROSS-pad extent, which differs from the grasped
        # width on a cuboid.
        L = CYL_L
        D = float(self._picked_band_width() or CYL_D) \
            if self._picked_shape() in ("cuboid", "cube") else CYL_D
        ...
        ax.plot(obj[1] + rot[:, 0], obj[2] + rot[:, 1], color="steelblue",
                linewidth=2,
                label=f"{self._picked_shape()} (tilt {cfg['tilt_deg']:.0f}° "
                      f"about {cfg['tilt_axis']})")
        ...
        # The 3D body follows the shape too. A box is drawn as a 4-sided
        # "cylinder" -- four angular samples on a square cross-section -- so the
        # same tilt and surface code below is reused unchanged rather than
        # forked for a second shape.
        zc = np.linspace(-CYL_L/2, CYL_L/2, 20)
        if self._picked_shape() in ("cuboid", "cube"):
            try:
                _hx = (float(self.vars["box_w"].get())
                       if self.vars["object_pick"].get() == self.OBJ_BOX
                       else CYL_D) / 2.0
            except ValueError:
                _hx = CYL_D / 2.0
            _hy = float(self._picked_band_width() or CYL_D) / 2.0
            _sq = np.array([[-_hx, -_hy], [_hx, -_hy], [_hx, _hy],
                            [-_hx, _hy], [-_hx, -_hy]])
            TH, ZC = np.meshgrid(np.arange(5), zc)
            XC = _sq[:, 0][TH]
            YC = _sq[:, 1][TH]
        else:
    ...
    def _sync_object_size(self, fnum):
        ...
        global CYL_D, CYL_L
        # SEPARATE FIELDS PER SHAPE (2026-08-28). The cuboid boxes used to
        # write into obj_diam/obj_len, so typing a cuboid width silently
        # changed the cylinder's diameter and you could not tell which set was
        # in force. Each shape now owns its own entries and only the SELECTED
        # one feeds CYL_D / CYL_L.
        if self.vars["object_pick"].get() == self.OBJ_BOX:
            d = fnum("box_w", 40.0)
            L = fnum("box_h", 100.0)
        else:
        ...
        _sh = self._picked_shape()
        _k = self._cal_key(_sh, CYL_D)
        ent = cal.get(_k)
        _borrowed = False
        _fn = os.path.basename(_cp)
        if _borrowed:
            # Visible, not silent. A fixed-angle close_rad transfers between
            # shapes (same jaw gap); a contact-aware one does not, because a
            # flat face trips the target far earlier than a round one.
            self.obj_cal_lbl.config(
                text=(f"\u00d8{CYL_D:.1f} {_sh}: NO ENTRY — using the "
                      f"CYLINDER calibration for this width "
                      f"(close_rad {ent.get('close_rad')}). Fine if it was "
                      f"measured fixed-angle; WRONG if contact-aware. "
                      f"Calibrate this shape."),
                foreground="#a60")
        elif ent:
            self.obj_cal_lbl.config(
                text=f"{_sh} {CYL_D:.1f} mm calibrated  "
            ...
            _also = ""
            if _sh != "cylinder" and f"{CYL_D:.1f}" in cal:
                _also = (f"  A CYLINDER entry for this width exists and is "
                         f"deliberately NOT used: same jaw gap, but a "
                         f"contact-aware close_rad measured on a round face "
                         f"is the wrong number for a flat one.")
            self.obj_cal_lbl.config(
                text=f"{_sh} {CYL_D:.1f} mm NOT in {_fn} - collection will "
                     f"refuse. ({why}). Calibrate tab first; try "
                     f"GRASP_CLOSE_RAD={est:.3f}.{_also}",
                foreground="#b00")
    ...
    def refresh_calib(self):
        ...
        ax.set_title(f"CALIBRATE — pad on {self._picked_shape()} (FRONT Y-Z)")
        ax.set_xlabel("world Y (mm)"); ax.set_ylabel("world Z (mm)")
        tilt = np.radians(cfg["tilt_deg"])
        # This view's horizontal axis is world Y, so the width to draw is the
        # ACROSS-pad extent — which differs from the grasped width on a cuboid,
        # and left this panel showing a cylinder for a box.
        _shp = self._picked_shape()
        L = CYL_L
        D = (float(self._picked_band_width() or CYL_D)
             if _shp in ("cuboid", "cube") else CYL_D)
        corners = np.array([[-D/2, -L/2], [D/2, -L/2], [D/2, L/2], [-D/2, L/2], [-D/2, -L/2]])
        c, s = np.cos(tilt), np.sin(tilt); Rt = np.array([[c, -s], [s, c]]); rot = corners @ Rt.T
        ax.plot(obj[1] + rot[:, 0], obj[2] + rot[:, 1], color="steelblue",
                linewidth=2, label=f"{_shp} {D:g} x {L:g} mm")
        ax.fill(obj[1] + rot[:, 0], obj[2] + rot[:, 1], color="steelblue", alpha=0.35)
    ...
    def save_and_show_calib_cmd(self):
        ...
        def _one(contact, suffix):
            ...
            return (
                ...
                f'GRASP_HEADLESS="{headless}" \\\n'
                f'GRASP_CALIBRATE="1" \\\n'
                # so the entry is stored under this SHAPE's key
                + (f'GRASP_OBJECT_SHAPE="{self._picked_shape()}" \\\n'
                   if self._picked_shape() != "cylinder" else "")
                + env
    ...
    def show_calibration(self):
        ...
        key = self._cal_key(self._picked_shape(), CYL_D)
        if key in cal:
            v = cal[key]
    ...
    def do_design_grid(self):
        ...
        try:
            ...
            # THE ACROSS ANCHOR IS THE USER'S (2026-08-27). Zeroing it centred
            # every sweep on the object, which is right for a narrow body but
            # wrong for a wide face: the 96 mm canvas then caps the sweep in
            # the middle and the stitched map is a featureless plateau with no
            ...
            self.design_lbl.config(
                text=(f"\u00d8{CYL_D:.0f}x{CYL_L:.0f}: "
                      ...
                      f"{res['pad_dz_mm']:+.1f} mm, y "
                      f"{self.vars['pad_dy'].get()}\n"
