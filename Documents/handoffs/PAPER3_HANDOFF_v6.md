# PAPER 3 — HANDOFF v6
**Supersedes v1–v5. Date: 2026-07-23.**
...
**9.6 — Temporal snapshot #4.** `WAIT_HOLD_SECONDS = 1.0` but Roberge's
convention needs a ~3 s hold, so snapshot #4 currently falls after release
and is flagged "needs 3s hold". Bump to 3.0 before dataset generation.
...
- [ ] `WAIT_HOLD_SECONDS` 1.0 → 3.0 for temporal snapshot #4 (§9.6)
...
*End of Handoff v6. Nothing from v1–v5 that still matters has been dropped.*
